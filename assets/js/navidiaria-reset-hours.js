...
(() => {
  ...
  function buildOfficialSheet() {
    ...
    const body = rows.map(row => {
      ...
      const cells = values.map((value,index) => `<td class="${index+1>maxDay?'outside-month':''}">${index+1>maxDay?'':html(formatValue(row,value))}</td>`).join('');
