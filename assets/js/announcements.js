(function(){
  ...
  async function setupAdmin(){
    ...
    try{
      ...
      selectedAgentId=recipients[0]?.id||'';
    ...
    const personalCard=()=>{
      ...
      const options=recipients.length?recipients.map(item=>'<option value="'+escapeHtml(item.id)+'"'+(item.id===selectedAgentId?' selected':'')+'>'+escapeHtml(item.name)+' · ID '+escapeHtml(item.id)+'</option>').join(''):'<option value="">Nessun agente registrato</option>';
    ...
    render();
    grid.addEventListener('change',event=>{const select=event.target.closest('[data-personal-target]');if(!select)return;selectedAgentId=select.value;render()});
