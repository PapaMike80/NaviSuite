...
function markCloudDirty(){const current=JSON.stringify(entries);if(current!==lastKnownEntriesSnapshot){undoSnapshot=lastKnownEntriesSnapshot;lastKnownEntriesSnapshot=current;setUndoUi(true)}localStorage.setItem(FIREBASE_DIRTY_KEY,'1');setSaveUi('dirty')}
function undoLastDiariaChange(){if(!undoSnapshot)return;entries=JSON.parse(undoSnapshot);lastKnownEntriesSnapshot=JSON.stringify(entries);undoSnapshot=null;localStorage.setItem(STORAGE,lastKnownEntriesSnapshot);localStorage.setItem(FIREBASE_DIRTY_KEY,'1');setUndoUi(false);setSaveUi('dirty');render();notify('Ultima modifica annullata')}
...
$('monthlySave')?.addEventListener('click',async()=>{try{await saveEntriesToFirebase();notify('Modifiche salvate su Firebase')}catch(error){notify('Salvataggio non riuscito')}});
$('weeklySync')?.addEventListener('click',syncDiariaNow);
$('weeklySave')?.addEventListener('click',async()=>{try{await saveEntriesToFirebase();notify('Modifiche salvate su Firebase')}catch(error){notify('Salvataggio non riuscito')}});
