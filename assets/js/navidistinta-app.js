...
function renderTodaySummary(){
  ...
  const shift=shiftFor(entry.shift),bp=shift.meal?(entry.mealUsed?'USATO':'DA ACCR.'):'—',diaria=entry.allowanceRate!==null&&entry.allowanceRate!==undefined?`${entry.allowanceRate}%`:'—',bankTotal=(Number(entry.bank)||0)+travelCredit(entry),id=escapeHtml(String(entry.id));
...
function totalRfBalance(){return entries.reduce((sum,e)=>sum+(Number(e.rf)||0),0)}
function render(){
  ...
  const weeks={};
  actualEntries.forEach(e=>{const s=shiftFor(e.shift),mins=effectiveWorkedMinutes(e);total+=mins;bank+=(Number(e.bank)||0)+travelCredit(e);embark+=e.embark?1:0;if(s.meal){e.mealUsed?mealsUsed++:meals++}allowances+=e.allowanceRate!==null&&e.allowanceRate!==undefined?1:0;const w=isoWeek(e.date);weeks[w]=(weeks[w]||0)+mins});
