...
async function syncCurrentAgent(force=false){
  ...
  try{
    ...
    Object.entries(agent.turni||{}).forEach(([date,raw])=>{
      ...
      else{const competence=shiftFor(shift);entries.push({id:`naviturni-${date}`,date,shift,travel,variationFrom,variationTo:variationFrom?shift:null,delay:0,overtimeComponents:window.NaviDiariaOvertime.create(),bank:0,rf:false,embark:!!competence.embark,cashHandling:cashDuty,cashHandlingManual:false,mealUsed:!!competence.meal,allowanceRate:competence.allowance?competence.allowanceRate:null,note:'',imported:true});added++}
    });
