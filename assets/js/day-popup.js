(function(){
  ...
  function handleClick(e){if(e.target===e.currentTarget||e.target.closest('[data-bubble-close]')){close();return}if(e.target.closest('[data-bubble-diaria]')){const date=draft?.date||opts?.date||'';location.href=`navidiaria.html${date?`?editDate=${encodeURIComponent(date)}`:''}`;return}const field=e.target.closest('[data-bubble-field]');if(!field||field.disabled)return;const row=[shiftRow,...primary,...other].find(item=>item.key===field.dataset.bubbleField);if(!row)return;if(['toggle','ticket','holiday','hydrofoil'].includes(row.kind))toggle(row);else edit(row)}
  async function open(next){const current=++token;opts=next;day=new Date(`${opts.date}T12:00:00`);label=new Intl.DateTimeFormat('it-IT',{weekday:'long',day:'numeric',month:'long'}).format(day);const m=modal(),showDiaria=opts.showDiariaButton!==false;m.querySelector('[data-bubble-title]').textContent=label;m.querySelector('.weekly-service').className='weekly-service';m.querySelector('.weekly-service').innerHTML='<strong>Caricamento…</strong>';m.querySelector('[data-bubble-grid]').innerHTML='';m.querySelector('[data-bubble-diaria]').hidden=!showDiaria;m.querySelector('.monthly-bubble-actions').style.gridTemplateColumns=showDiaria?'1fr 1fr':'1fr';m.hidden=false;document.body.classList.add('weekly-dialog-open');const loaded=await opts.loadEntry();if(current!==token||!opts)return;draft={...loaded};render()}
